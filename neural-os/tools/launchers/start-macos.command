...
if [ ! -f "$NODE" ]; then
  ...
  if [ -n "$SYSTEM_NODE" ] && [ "$SYSTEM_MAJOR" -ge 20 ] 2>/dev/null; then
    ...
    echo "    1. Stick in einen Mac stecken, auf dem Neural OS schon läuft, und"
    echo "       dort unter Einstellungen -> Stick auf \"Stick aktualisieren\""
    echo "       klicken. Die Laufzeit des jeweiligen Rechners wird immer"
    echo "       mitkopiert."
