...
if [ ! -f "$NODE" ]; then
  ...
  if [ -n "$SYSTEM_NODE" ] && [ "$SYSTEM_MAJOR" -ge 20 ] 2>/dev/null; then
    ...
    echo "       läuft. Dort in der Seitenleiste den Bereich \"Stick\" öffnen"
    echo "       (oder g dann t), den Pfad des Sticks eintragen und unter"
    echo "       \"Welche Rechner der Stick starten kann\" bei diesem System"
    echo "       auf \"Jetzt kopieren\" klicken. Das braucht kein Internet."
