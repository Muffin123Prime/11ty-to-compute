    ...
    await self.api.stream(route, {
      ...
      onEvent: (event) => {
        ...
        if (event.type === 'fortschritt') {
          ...
          lauf.fehler = (nutz.error && nutz.error.message) || 'Der Vorgang ist gescheitert.';
        }
  ...
  return karte('Welche Rechner der Stick starten kann',
    h('div.stack', null,
      h('p', null, text(
        ...
        + 'Jede weitere Laufzeit ist ein einmaliger Download des offiziellen Node-Pakets durch die '
        + 'Netzschleuse – mit Prüfsummenvergleich. Verweigert die Schleuse ihn, ist das kein Fehler '
        + 'des Sticks: er läuft weiterhin auf deinem eigenen Betriebssystem.')),
...
function wahrheitenBlock(self) {
  ...
  /* --- das Modell -------------------------------------------------- */
  zeilen.push(wahrheit('Das Sprachmodell kommt NICHT mit auf den Stick',
    (modell && modell.grund)
      || 'Ein Sprachmodell gehört einem Anbieter auf diesem Rechner, nicht Neural OS.',
    'fehlt'));
  zeilen.push(h('p.meta.stickv__folge', null, text(
    'Konkret: auf einem fremden Rechner ohne eigenes Modell hast du alle Notizen, Chats, Projekte und '
    + 'Verknüpfungen – aber keine neuen Antworten. Das Wissen reist mit, das Denken nicht. '
    + (modell && modell.geprueft
      ? (modell.hierErreichbar
        ? 'Auf diesem Rechner ist ein Modell erreichbar; auf dem Stick liegt es deshalb trotzdem nicht.'
        : 'Auf diesem Rechner ist zurzeit ohnehin kein Modell erreichbar.')
